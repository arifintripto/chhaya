...
if($seach_user_id && $report_type && $hierarchy_level){
...
$fist_day = Carbon::now()->startOfYear();
$now  = Carbon::now();
...
<div class="row">
    <div class="col-md-12 text-center mt-4 mb-4 text-uppercase">
        <h2>This year Statistics @if($checked_user) <br><span>For <br> <strong style="color:#dc3545">{{$checked_user->first_name}}</strong></span>@endif</h2>
    </div>
</div>
<div class="row mb-5">
    <form action="" style="width: 100%;background: #f1f1f1;padding: 20px;" class="form-inline">
        ...
        <div class="col-md-4 pr-1 pl-1">
            <select name="user_id" id="select_user" class="form-control border-0 rounded-0" style="width: 100%;" required>
                <option value="-1" disabled>-- Select Hierarchy First--</option>
                @if($hierarchy_level)
                @php 
                
                $users = DB::table('tb_users')->where('hierarchy_level',$hierarchy_level)->get(); 
